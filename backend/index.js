...
export default function HomeScreen() 
{
  ...
  // Update pendingReports count dynamically based on allReports
  React.useEffect(() => {
    const countPending = allReports.filter(report => report.status !== 'Ignored').length;
    setPendingReports(countPending);
  }, [allReports]);
  ...
  try {
    ...
    const aiResponse = await fetch('https://30ce-34-53-41-214.ngrok-free.app/analyze', {
      method: 'POST',
      body: formData,
      headers: {
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      await uploadToServer(); //upload without button
    } else {
  ...
  const validateAndLogin = async () => {
    ...
    try {
      const response = await fetch('https://79ea-183-82-237-45.ngrok-free.app/api/auth/login', {
        method: 'POST',
  ...
  const validateAndSignUp = async () => {
    ...
    try {
      const response = await fetch('https://79ea-183-82-237-45.ngrok-free.app/api/auth/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: fullName,
...
const sendOtpToEmail = async () => {
  ...
  try {
    const res = await fetch('https://79ea-183-82-237-45.ngrok-free.app/api/send-otp', { 
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const verifyOTP = async () => {
    try {
      const res = await fetch('https://79ea-183-82-237-45.ngrok-free.app/api/verify-otp', { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const uploadToServer = async () => {
    try {
      ...
      const analyzeRes = await fetch('https://79ea-183-82-237-45.ngrok-free.app/analyze', {
        method: 'POST',
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        xhr.open('POST', 'https://79ea-183-82-237-45.ngrok-free.app/api/upload/new');
  ...
  const resetPassword = async () => {
    ...
    try {
      const res = await fetch('https://79ea-183-82-237-45.ngrok-free.app/api/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const res = await fetch('https://79ea-183-82-237-45.ngrok-free.app/api/upload/all');
        const data = await res.json();
        setAllReports(data);
      } catch (err) {
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const response = await fetch('https://79ea-183-82-237-45.ngrok-free.app/generate-pdf', {
  ...
  const generateAndOpenPdf = async (html: string, fileName: string): Promise<void> => {
    try {
      const response = await fetch('https://79ea-183-82-237-45.ngrok-free.app/api/generate-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    const fetchUserHistory = async () => {
      try {
        ...
        const res = await fetch(`https://79ea-183-82-237-45.ngrok-free.app/api/upload/user/${userId}`);
        const data = await res.json();
        setHistoryData(data);
      } catch (error) {
  ...
  //       },
  //       body: JSON.stringify({
  //         imageUrl: `https://79ea-183-82-237-45.ngrok-free.app${report.imageUrl}`,
  ...
  return (
    <ImageBackground source={require('@/assets/images/potholeclick.png')} style={styles.background}>
          ...
          {historyData.length === 0 ? (
            ...
            historyData.map((item, index) => (
              ...
              >
                {item.imageUrl && (
                  <Image
                    source={{ uri: `https://79ea-183-82-237-45.ngrok-free.app${item.imageUrl}` }}
          ...
          >
            <View style={styles.supervisorDashboardContainer}>
              ...
              <View style={styles.cardRow}>
                ...
                {/* <View style={[styles.dashboardCard, { backgroundColor: 'rgba(40, 167, 69, 0.8)' }]}>
                  <ThemedText type="defaultSemiBold" style={styles.cardTitle}>Completed</ThemedText>
                ...
                {loadingReports ? (
                  ...
                  allReports.map((report, index) => (
                    <TouchableOpacity
                        ...
                        {report.imageUrl ? (
                            <Image
                            source={{ uri: `https://79ea-183-82-237-45.ngrok-free.app${report.imageUrl}` }}
                            style={{
                                width: 60,
                                height: 60,
                                borderRadius: 10,
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }}>
          <View style={{
            ...
            {selectedReport.imageUrl ? (
              <Image
                source={{ uri: `https://79ea-183-82-237-45.ngrok-free.app${selectedReport.imageUrl}` }}
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                try {
                  const location = selectedReport.location || 'Unknown Location';
                  const summary = selectedReport.summary || 'No summary available';
                  const status = selectedReport.status || 'Pending';
                  const date = selectedReport.createdAt ? new Date(selectedReport.createdAt).toLocaleDateString('en-GB') : 'Unknown Date';
                  ...
                  const html = `
                    <html>
                      ...
                      <body>
                        <h1>SafeStreet Damage Report Summary</h1>
                  ...
                  if (contentType.includes('application/json')) {
                    ...
                    if (response.ok && data.url) {
                      await downloadAndSavePdf(data.url);  
                    } else {
            ...
            {selectedReport.status !== 'Ignored' && (
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    const res = await fetch(`https://79ea-183-82-237-45.ngrok-free.app/api/upload/ignore/${selectedReport._id}`, {
                      method: 'PUT',
                      headers: { 'Content-Type': 'application/json' },
                    });
      ...
      {chatbotScreens.includes(screen) && <ChatbotIcon />}
      {/* <ChatbotIcon /> */}
    </ImageBackground>
  );
}
const styles = StyleSheet.create({
  ...
  loginCard: { width: '95%', maxWidth: 500, backgroundColor: 'rgba(0,0,0,0.7)', padding: 20, borderRadius: 15, alignItems: 'center',},
