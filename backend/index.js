...
//     const flaskURL = 'https://02ba-34-81-8-246.ngrok-free.app/analyze';
...
//     try {
//       const geo = await axios.get('https://0be2-183-82-237-45.ngrok-free.app/search', {
...
//     const subject = "New Report Received";
//     const text = `Location: ${location}\nSummary: ${summary}\nStatus: ${status}`;
//     const html = `<h2>New Damage Report</h2><p><strong>Location:</strong> ${location}</p><p><strong>Summary:</strong> ${summary}</p><p><strong>Status:</strong> ${status}</p>`;

//     if (typeof sendEmail === "function") {
//       await sendEmail(adminEmail, subject, text, html);
//     }

//     res.status(200).json({ message: "Report received and stored successfully" });
//   } catch (error) {
...
app.post('/analyze', upload.single('image'), async (req, res) => {
  try {
    ...
    const flaskURL = 'https://55f1-35-245-41-189.ngrok-free.app/analyze';
    const response = await axios.post(flaskURL, form, {
      headers: { ...form.getHeaders() },
    });
...
app.post('/api/upload/new', upload.single('image'), async (req, res) => {
  try {
    ...
    try {
      const geo = await axios.get('https://26c3-183-82-106-156.ngrok-free.app/search', {
        params: { q: location, format: 'json', limit: 1 },
        headers: { 'User-Agent': 'SafeStreetApp/1.0 (youremail@example.com)' }
      });
